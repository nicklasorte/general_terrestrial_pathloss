function [array_full_pl_data,full_prop_mode_data,full_rx_angle_data,full_tx_angle_data]=load_full_pathloss_info_angles_rev4(app,data_label1,string_prop_model,grid_spacing,tf_recalculate,base_protection_pts,sim_array_list_bs,sim_number,tf_tropo_cut)
                    ...
                    if strcmp(string_prop_model,'TIREM')
                        ...
                        'Error: Check this prop model for angles'
                        pause;
                    end
                    ...
                    filename_full_tx_angle_data=strcat(data_label1,'_full_tx_angle_data_',string_prop_model,'_',num2str(grid_spacing),'km.mat');
                    [var_exist_full_tx_angle_data]=persistent_var_exist_with_corruption(app,filename_full_tx_angle_data);

                    if tf_recalculate==1
                        var_exist_full_prop_mode_data=0;
                    end

                    if var_exist_full_prop_mode_data==2 && var_exist_full_rx_angle_data==2 && var_exist_array_full_pl_data==2 && var_exist_full_tx_angle_data==2
                        retry_load=1;
                        while(retry_load==1)
                            try
                                tic;
                                ...
                                load(filename_full_tx_angle_data,'full_tx_angle_data')
                                toc;
                                pause(0.1)
                                retry_load=0;
                            catch
...
% % %                             end
                        ...
                        while(retry_save==1)
                            try
                                ...
                                save(filename_full_tx_angle_data,'full_tx_angle_data')
                                pause(0.1)
                                retry_save=0;
                            catch
                                retry_save=1;
